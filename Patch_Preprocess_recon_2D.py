...
def Patch_Preprocess_recon_2D(patch_size_x=5,patch_size_y=5,in_root='',out_root='',recon_flag=True):
    ...
    patches = np.zeros(patch_pixels*5)
...
#    print
    ...
    if 'training' in out_root and recon_flag == True:
        ...
        np.save(out_root+'trainpatch_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_T.npy',patches)
        np.save(out_root+'trainlabel_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_T.npy',ground_truth)
    elif recon_flag == True:
        print '... Saving the 2D validation patches'
        np.save(out_root+'validpatch_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_T.npy',patches)
        np.save(out_root+'validlabel_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_T.npy',ground_truth)
    
    if 'training' in out_root and recon_flag == False:
        print'... Saving the 2D training patches'
        np.save(out_root+'trainpatch_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_F.npy',patches)
        np.save(out_root+'trainlabel_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_F.npy',ground_truth)
        
    elif recon_flag == False:
        print '... Saving the 2D validation patches'
        np.save(out_root+'validpatch_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_F.npy',patches)
        np.save(out_root+'validlabel_2D_'+str(patch_size_x)+'_'+str(patch_size_y)+'_F.npy',ground_truth)